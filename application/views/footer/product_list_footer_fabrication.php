...
<script>
    var table = $("#table-product-fabrication").DataTable({
        ajax: {
            ...
            dataSrc: function(data) {
                return data.map(function(order) {
                    ...
                    if (order.status == "waiting") {
                        ...
                        action += "<button onClick='deleteOrderToFabrication(" + order.id + ")' class='btn btn-danger'>Hapus</button>";
                    }
    ...
    function confirmUpdate() {
        ...
        $.ajax({
            url: "order/update_Order_Fabrication",
            method: "POST",
            data: {
                id: id,
                quantity: qty
    ...
    function deleteOrderToFabrication(id) {
        if (confirm("Are you sure to delete this component order?")) {
            $(".se-pre-con").fadeIn("fast");
            $.ajax({
                url: "order/delete_Order_Fabrication",
                method: "POST",
                data: {
                    id: id
                },
